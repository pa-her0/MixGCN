...
class GATLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.conv = GATConv(
            ...
            heads=heads,
            concat=concat,
            dropout=attn_dropout,       # 注意：GATConv 的 dropout 作用于注意力与特征
    ...
    def forward(self, x, edge_index, edge_weight=None):  # 保留 edge_weight 以兼容统一接口
...
class MixGATLayer(nn.Module):
    """
    X_{l+1} = beta * Z + (C - beta) * sigma(Z),  Z = GAT(X_l)
    """
    def __init__(
        self,
        in_dim: int,
    ...
    def forward(self, x, edge_index, edge_weight=None):  # 兼容签名
        x = self.feat_dropout(x)
    ...
    def __init__(
        ...
        *,
        ...
        for l in range(layers):
            is_last = (l == layers - 1)
            if is_last:
                # 最后一层：heads=1, concat=False 输出 logits
                layer = GATLayer(
...
def main():
    ...
    parser.add_argument('--beta_list', type=str, default="0.95,0.88,0.82",
    ...
    parser.add_argument('--leaky_slope', type=float, default=0.2, help='LeakyRelu 负斜率（注意力得分）')
