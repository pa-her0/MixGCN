...
# Baseline-JK & Mix-JK on WikiCS / Coauthor-CS / Coauthor-Physics
...
class ConvWrapper(nn.Module):
    """统一 forward(x, edge_index, edge_weight) 接口；GAT 忽略 edge_weight。"""
    def __init__(self, conv, kind: str):
        super().__init__()
        self.conv = conv
        self.kind = kind
...
def make_conv(in_dim: int, out_dim: int, kind: str, *, heads: int = 1,
              attn_dropout: float = 0.0, leaky_slope: float = 0.2, concat: bool = True):
    if kind == 'gcn':
        return ConvWrapper(GCNConv(in_dim, out_dim, cached=False, normalize=True, bias=True), 'gcn')
    if kind == 'sage':
        return ConvWrapper(SAGEConv(in_dim, out_dim, bias=True), 'sage')
    if kind == 'gat':
        return ConvWrapper(GATConv(in_dim, out_dim, heads=heads, concat=concat,
                                   dropout=attn_dropout, negative_slope=leaky_slope,
                                   add_self_loops=True, bias=True), 'gat')
    raise ValueError(f'Unknown conv kind: {kind}')

def get_layer_out_dim(base_out: int, kind: str, heads: int, concat: bool):
    if kind == 'gat' and concat:
        return base_out * heads
    return base_out

# -----------------------------
# 单层：Baseline 与 Mix（在 JK 之前）
# -----------------------------
class BaseLayer(nn.Module):
    def __init__(self, in_dim, out_dim, *, conv_kind='gcn', heads=1, concat=True,
                 feat_dropout=0.5, attn_dropout=0.0, leaky_slope=0.2, activation=F.leaky_relu):
        super().__init__()
        ...
        self.concat = concat
        self.heads = heads
        self.conv = make_conv(in_dim, out_dim, conv_kind, heads=heads,
                              attn_dropout=attn_dropout, leaky_slope=leaky_slope, concat=concat)
        self.out_dim = get_layer_out_dim(out_dim, conv_kind, heads, concat)
...
class MixLayer(nn.Module):
    """
    Z = Conv(X)；X' = beta * Z + (C - beta) * sigma(Z)
    """
    def __init__(self, in_dim, out_dim, *, beta=0.5, C=1.2,
                 conv_kind='gcn', heads=1, concat=True,
                 feat_dropout=0.5, attn_dropout=0.0, leaky_slope=0.2, activation=F.leaky_relu):
        super().__init__()
        self.beta = float(beta); self.C = float(C)
        self.feat_dropout = nn.Dropout(feat_dropout)
        self.activation = activation
        self.conv_kind = conv_kind
        self.concat = concat
...
# JK-Net 主干
# -----------------------------
class BaselineJK(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=4, dropout=0.5, conv_kind='gcn',
                 heads_hidden=1, attn_dropout=0.0, leaky_slope=0.2,
                 activation=F.leaky_relu, jk_mode='cat'):
        super().__init__()
        assert layers >= 2
        self.layers = nn.ModuleList()
        cur_in = in_dim
        for l in range(layers-1):
            layer = BaseLayer(cur_in, hidden, conv_kind=conv_kind,
                              heads=heads_hidden, concat=True if conv_kind=='gat' else True,
                              feat_dropout=dropout, attn_dropout=attn_dropout,
                              leaky_slope=leaky_slope, activation=activation)
            self.layers.append(layer)
            cur_in = layer.out_dim
        # 最后一层（仍作为“隐层”参与 JK）
        last = BaseLayer(cur_in, hidden, conv_kind=conv_kind,
                         heads=heads_hidden, concat=True if conv_kind=='gat' else True,
                         feat_dropout=dropout, attn_dropout=attn_dropout,
                         leaky_slope=leaky_slope, activation=activation)
        self.layers.append(last)

        per_layer_dim = last.out_dim
        self.jk = JumpingKnowledge(jk_mode, channels=per_layer_dim, num_layers=layers)
        agg_dim = per_layer_dim * layers if jk_mode == 'cat' else per_layer_dim
        self.head = nn.Linear(agg_dim, out_dim)
    ...
    def forward(self, x, edge_index, edge_weight=None, return_hidden=False):
        ...
        for layer in self.layers:
            ...
            feats.append(x)
        h_jk = self.jk(feats)          # 聚合后的隐表示
        logits = self.head(h_jk)
        return (logits, h_jk) if return_hidden else logits

class MixJK(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=4, dropout=0.5, beta=0.9, C=1.0, beta_list: Optional[List[float]] = None,
                 conv_kind='gcn', heads_hidden=1, attn_dropout=0.0, leaky_slope=0.2,
                 activation=F.leaky_relu, jk_mode='cat'):
        super().__init__()
        assert layers >= 2
        self.layers = nn.ModuleList()
        ...
        for l in range(layers):
            ...
            mod = MixLayer(cur_in, hidden, beta=beta_l, C=C, conv_kind=conv_kind,
                           heads=heads_hidden, concat=True if conv_kind=='gat' else True,
                           feat_dropout=dropout, attn_dropout=attn_dropout,
                           leaky_slope=leaky_slope, activation=activation)
        ...
        per_layer_dim = self.layers[-1].out_dim
        self.jk = JumpingKnowledge(jk_mode, channels=per_layer_dim, num_layers=layers)
        agg_dim = per_layer_dim * layers if jk_mode == 'cat' else per_layer_dim
        self.head = nn.Linear(agg_dim, out_dim)
...
def load_wikics(root: str, split_id: int = 0, undirected: bool = True) -> Tuple[Data, torch.Tensor, Optional[torch.Tensor]]:
    ...
    assert data.train_mask.dim() == 2 and data.val_mask.dim() == 2
    assert 0 <= split_id < data.train_mask.size(1)
...
def _make_masks_perclass(y: torch.Tensor, num_classes: int, train_per_class: int, val_per_class: int, seed: int):
    ...
    if train_mask.sum() == 0 or val_mask.sum() == 0 or test_mask.sum() == 0:
        raise RuntimeError("perclass 划分为空，请调大样本数。")
...
def _make_masks_ratio(y: torch.Tensor, train_ratio: float, val_ratio: float, seed: int):
    ...
    if train_mask.sum() == 0 or val_mask.sum() == 0 or test_mask.sum() == 0:
        raise RuntimeError("ratio 划分为空，请调整比例。")
    return train_mask, val_mask, test_mask
...
def load_coauthor(root: str, which: str = 'CS', split_mode: str = 'perclass',
    ...
    data.train_mask = tr_mask; data.val_mask = va_mask; data.test_mask = te_mask
    return data, data.edge_index, None
...
def evaluate_single_graph(model, data, edge_index, edge_weight, split='val'):
    ...
    def _pick_mask(mask):
        if mask.dim() == 2: return mask[:, 0]
        return mask

    if split == 'val': mask = _pick_mask(data.val_mask)
    elif split == 'test': mask = _pick_mask(data.test_mask)
    else: mask = _pick_mask(data.train_mask)
...
class RunConfig:
    ...
    C: float = 1.0
    tau: float = 0.90
    abs_target: float = 0.85
    device: str = 'auto'
    beta_list: Optional[List[float]] = None
    beta_mode: str = 'custom'  # 与原版一致（保留接口）

    split_id: int = 0
    split_mode: str = 'perclass'
    ...
    heads_hidden: int = 4      # 仅 gat 有效
    attn_dropout: float = 0.0  # 仅 gat 有效
    leaky_slope: float = 0.2   # 仅 gat 有效
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        return BaselineJK(
            ...
            layers=cfg.layers, dropout=cfg.dropout, conv_kind=cfg.conv,
            heads_hidden=cfg.heads_hidden, attn_dropout=cfg.attn_dropout,
            leaky_slope=cfg.leaky_slope, activation=F.leaky_relu, jk_mode=cfg.jk_mode
        )
    elif cfg.model_type == 'mix':
        return MixJK(
            in_dim, cfg.hidden, out_dim,
            layers=cfg.layers, dropout=cfg.dropout, beta=cfg.beta, C=cfg.C, beta_list=betas,
            conv_kind=cfg.conv, heads_hidden=cfg.heads_hidden, attn_dropout=cfg.attn_dropout,
            leaky_slope=cfg.leaky_slope, activation=F.leaky_relu, jk_mode=cfg.jk_mode
        )
    else:
...
def train_one_run_single_graph(cfg: RunConfig, data: Data, edge_index, edge_weight) -> Tuple[Dict, pd.DataFrame]:
    ...
    model = build_model(cfg, data.num_features, num_classes).to(device)
    data = data.to(device); edge_index = edge_index.to(device)
    edge_weight = edge_weight.to(device) if edge_weight is not None else None

    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)

    best_val = -1.0; best_test_at_val = -1.0; best_epoch = 0; no_improve = 0
    rows = []; timer_started = False; wall_start = 0.0
    prev_grad_vec = None; prev_loss = None
    ...
    t = df['wall_time'].to_numpy(); a = df['val_acc'].to_numpy()
    ...
    a0 = float(df.iloc[0]['val_acc']); a_best = float(df['val_acc'].max())
...
def main():
    ...
    parser.add_argument('--abs_target', type=float, default=0.88)
    parser.add_argument('--seeds', type=int, nargs='+', default=[0,1,2])
    parser.add_argument('--outdir', type=str, default='../runs/data')

    # WikiCS 专用
    parser.add_argument('--split_id', type=int, default=15)
    ...
    parser.add_argument('--heads_hidden', type=int, default=4, help='仅 gat 有效')
    parser.add_argument('--attn_dropout', type=float, default=0.0, help='仅 gat 有效')
    parser.add_argument('--leaky_slope', type=float, default=0.2, help='仅 gat 有效')
    ...
    for model_type in ['mix','baseline']:
        for seed in args.seeds:
            cfg = RunConfig(
                ...
                jk_mode=args.jk_mode, conv=args.conv,
                heads_hidden=args.heads_hidden,
                attn_dropout=args.attn_dropout, leaky_slope=args.leaky_slope
            )
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{ds_name}_leaky_relu.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{ds_name}_leaky_relu.csv")
