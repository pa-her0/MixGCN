...
class SAGELayer(nn.Module):
    def __init__(self, in_dim, out_dim, *, dropout=0.5, activation=F.elu, aggr='mean', bias=True):
        super().__init__()
...
class MixSAGELayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, *, beta=0.5, C=1.2,
                 dropout=0.5, activation=F.elu, aggr='mean', bias=True):
        super().__init__()
...
class BaselineSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=2, dropout=0.5, activation=F.elu, aggr='mean'):
        super().__init__()
        assert layers >= 2
        mods = []
...
class MixSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 ...
                 activation=F.elu, beta_list: Optional[List[float]] = None, aggr='mean'):
        super().__init__()
        assert layers >= 2
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        return BaselineSAGE(
            ...
            activation=F.elu, aggr=cfg.sage_aggr
        )
...
def main():
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{ds_name}_SAGE_GRELU.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{ds_name}_SAGE_GRELU.csv")
