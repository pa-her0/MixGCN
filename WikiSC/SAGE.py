...
# Baseline-SAGE vs MixSAGE on WikiCS / Coauthor-CS / Coauthor-Physics
...
class SAGELayer(nn.Module):
    def __init__(self, in_dim, out_dim, *, dropout=0.5, activation=F.leaky_relu, aggr='mean', bias=True):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.activation = activation
        # SAGEConv 不使用 edge_weight；接口里保留以兼容
        self.conv = SAGEConv(in_dim, out_dim, aggr=aggr, bias=bias)
...
class MixSAGELayer(nn.Module):
    """
    Z = SAGE(X)；X' = beta * Z + (C - beta) * sigma(Z)
    """
    def __init__(self, in_dim, out_dim, *, beta=0.5, C=1.2,
                 dropout=0.5, activation=F.leaky_relu, aggr='mean', bias=True):
        super().__init__()
...
# -----------------------------
# 模型：BaselineSAGE / MixSAGE
# -----------------------------
class BaselineSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=2, dropout=0.5, activation=F.leaky_relu, aggr='mean'):
        super().__init__()
        assert layers >= 2
        mods = []
...
class MixSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=2, dropout=0.5, beta=0.5, C=1.0,
                 activation=F.leaky_relu, beta_list: Optional[List[float]] = None, aggr='mean'):
        super().__init__()
        assert layers >= 2
        ...
        for l in range(layers):
            ...
            mods.append(MixSAGELayer(idim, odim, beta=beta_l, C=C, dropout=dropout, activation=act, aggr=aggr))
        self.layers = nn.ModuleList(mods)
...
class RunConfig:
    ...
    # SAGE 相关
    sage_aggr: str = 'mean'  # 'mean' | 'max' | 'add'(sum)
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        return BaselineSAGE(
            ...
            activation=F.leaky_relu, aggr=cfg.sage_aggr
        )
...
def main():
    ...
    parser.add_argument('--C', type=float, default=1.15)

    # 收敛指标阈值
    parser.add_argument('--tau', type=float, default=0.90)
    parser.add_argument('--abs_target', type=float, default=0.89)
    ...
    parser.add_argument('--directed', dest='undirected', action='store_false', help='保持有向')
    parser.set_defaults(undirected=True)

    # SAGE
    parser.add_argument('--sage_aggr', type=str, default='mean', choices=['mean','max','add'],
                        help='SAGE 聚合方式')
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{ds_name}_SAGE_leaky_relu.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{ds_name}_SAGE_leaky_relu.csv")
