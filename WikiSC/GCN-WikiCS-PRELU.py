...
class GCNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=None, cached: bool = True):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=cached, normalize=True, bias=True)
        self.dropout = nn.Dropout(dropout)
        # 用 learnable PReLU 替代 F.elu
        self.activation = activation or nn.PReLU(out_dim)
...
class MixGCNLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.2, dropout=0.5, activation=None, cached: bool = True):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=cached, normalize=True, bias=True)
...
class BaselineGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, cached: bool = True):
        super().__init__()
        assert layers >= 2
        mods = []
        ...
        for l in range(layers):
            ...
            act = nn.PReLU(odim) if l < layers - 1 else None
