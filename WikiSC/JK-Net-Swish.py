...
class Swish(nn.Module):
    ...
    def __init__(self, learnable: bool = False):
        ...
        if learnable:
            self.beta = nn.Parameter(torch.tensor(1.0))  # learnable β
        else:
...
class BaseLayer(nn.Module):
    def __init__(self, in_dim, out_dim, *, conv_kind='gcn', heads=1, concat=True,
                 feat_dropout=0.5, attn_dropout=0.0, leaky_slope=0.2,
                 activation=None):
        super().__init__()
        self.feat_dropout = nn.Dropout(feat_dropout)
        # ✅ 默认使用 Swish 激活函数（平滑非线性）
    ...
    def __init__(self, in_dim, hidden, out_dim, *,
        ...
        for l in range(layers - 1):
            layer = BaseLayer(cur_in, hidden, conv_kind=conv_kind,
                              ...
                              leaky_slope=leaky_slope, activation=activation or Swish())
...
class MixJK(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, *,
                 layers=4, dropout=0.5, beta=0.9, C=1.0, beta_list=None,
...
def main():
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{ds_name}_Swish.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{ds_name}_Swish.csv")
