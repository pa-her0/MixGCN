...
class Swish(nn.Module):
    def __init__(self, learnable: bool = False):
        """
        learnable=True 时变为 SiLU(βx)=x*sigmoid(βx)
        learnable=False 时 β=1
        """
        super().__init__()
...
class GCNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=None, cached: bool = True):
        ...
        self.activation = activation or Swish()  # 默认 Swish
...
class MixGCNLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.2, dropout=0.5,
                 activation=None, cached: bool = True):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=cached, normalize=True, bias=True)
...
class BaselineGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5,
                 layers=2, cached: bool = True, learnable_swish=False):
        super().__init__()
        assert layers >= 2
        ...
        for l in range(layers):
            ...
            act = Swish(learnable=learnable_swish) if l < layers - 1 else None
...
class MixGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
                 ...
                 cached: bool = True, learnable_swish=False):
        super().__init__()
        assert layers >= 2
        dims = [in_dim] + [hidden] * (layers - 1) + [out_dim]
