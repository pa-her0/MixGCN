...
class SAGELayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=F.relu, aggr: str = 'mean'):
        super().__init__()
...
class MixSAGELayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.0, dropout=0.5, activation=F.relu, aggr: str = 'mean'):
        super().__init__()
        self.conv = SAGEConv(in_dim, out_dim, aggr=aggr, bias=True)
...
class BaselineSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=F.relu, aggr: str = 'mean'):
        super().__init__()
        assert layers >= 2
        mods = []
...
class MixSAGE(nn.Module):
    ...
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
                 beta=0.5, C=1.0, activation=F.relu, beta_list: Optional[List[float]] = None, aggr: str = 'mean'):
        super().__init__()
        assert layers >= 2
        dims = [in_dim] + [hidden] * (layers - 1) + [out_dim]
...
def main():
    ...
    parser.add_argument('--layers_list', type=str, default='',
    ...
    for L in layers_to_run:
        print(f"\n========== Running SAGE at layers = {L} ==========")

        # 为当前 L 构造 / 校验 beta_list
        ...
        # 文件名带上 L
        summary_csv = os.path.join(args.outdir, f"summary_SAGE_{args.dataset}_relu_L{L}.csv")
        curves_csv = os.path.join(args.outdir, f"curves_SAGE_{args.dataset}_relu_L{L}.csv")
