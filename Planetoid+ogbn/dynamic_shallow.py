...
import matplotlib.pyplot as plt  # 新增：用于训练动态可视化
...
# -----------------------------
# 模型定义：Baseline 与 MixGCN
# -----------------------------
class GCNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=F.relu):
        super().__init__()
        # 使用 gcn_norm 预归一化过的边/权，因此 normalize=False。
...
class MixGCNLayer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        if self.activation is None:
            # 最后一层：C * z，仅缩放 logits，不改变 argmax
            return self.C * z
        return self.beta * z + (self.C - self.beta) * self.activation(z)


class BaselineGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=F.relu):
        super().__init__()
        assert layers >= 2
        mods = []
        ...
        for l in range(layers):
            ...
            mods.append(GCNLayer(idim, odim, dropout=dropout, activation=act))
...
class MixGCN(nn.Module):
    ...
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
                 beta=0.5, C=1.0, activation=F.relu, beta_list: Optional[List[float]] = None):
        super().__init__()
        assert layers >= 2
        dims = [in_dim] + [hidden] * (layers - 1) + [out_dim]
...
# -----------------------------
# 训练动态可视化（Baseline vs Mix，按 epoch 聚合多 seed）
# -----------------------------
def plot_dynamics_for_layers(curves_df: pd.DataFrame, dataset: str, L: int, outdir: str):
    """
    针对某个 layers = L，把 Baseline vs Mix 的训练动态画出来：
      - loss vs epoch
      - val_acc vs epoch
      - grad_norm vs epoch
    多个 seed 会在 epoch 维度上做平均，并画上 ±1 std 的阴影。
    """
    os.makedirs(outdir, exist_ok=True)

    df = curves_df.copy()

    # 按 model + epoch 聚合（不同 seed）
    grouped = df.groupby(['model', 'epoch'])
    stat = grouped.agg({
        'loss': ['mean', 'std'],
        'val_acc': ['mean', 'std'],
        'grad_norm': ['mean', 'std'],
    ...
    base = stat[stat['model'] == 'baseline']
    mix = stat[stat['model'] == 'mix']

    if base.empty or mix.empty:
        print(f"[Plot] Skip dynamics for {dataset}, L={L}: baseline or mix missing.")
        return
    ...
    # 1) Loss
    ax = axes[0]
    ax.plot(base['epoch'], base['loss_mean'], label='Baseline', linestyle='--')
    ax.fill_between(base['epoch'],
                    base['loss_mean'] - base['loss_std'],
                    base['loss_mean'] + base['loss_std'],
                    alpha=0.2)
    ax.plot(mix['epoch'], mix['loss_mean'], label='Mix', linestyle='-')
    ax.fill_between(mix['epoch'],
                    mix['loss_mean'] - mix['loss_std'],
                    mix['loss_mean'] + mix['loss_std'],
                    alpha=0.2)
    ax.set_ylabel('Train Loss')
    ax.legend()
    ax.set_title(f'{dataset}, L={L}: Loss / Val / Grad vs Epoch')

    # 2) Val Acc
    ax = axes[1]
    ax.plot(base['epoch'], base['val_mean'], label='Baseline', linestyle='--')
    ax.fill_between(base['epoch'],
                    base['val_mean'] - base['val_std'],
                    base['val_mean'] + base['val_std'],
                    alpha=0.2)
    ax.plot(mix['epoch'], mix['val_mean'], label='Mix', linestyle='-')
    ax.fill_between(mix['epoch'],
                    mix['val_mean'] - mix['val_std'],
                    mix['val_mean'] + mix['val_std'],
                    alpha=0.2)
    ...
    # 3) Grad Norm
    ax = axes[2]
    ax.plot(base['epoch'], base['grad_mean'], label='Baseline', linestyle='--')
    ax.fill_between(base['epoch'],
                    base['grad_mean'] - base['grad_std'],
                    base['grad_mean'] + base['grad_std'],
                    alpha=0.2)
    ax.plot(mix['epoch'], mix['grad_mean'], label='Mix', linestyle='-')
    ax.fill_between(mix['epoch'],
                    mix['grad_mean'] - mix['grad_std'],
                    mix['grad_mean'] + mix['grad_std'],
                    alpha=0.2)
    ...
    plt.tight_layout()
    fig_path = os.path.join(outdir, f'dynamics_{dataset}_L{L}.png')
    plt.savefig(fig_path, dpi=300)
    plt.close(fig)
    print(f"[Plot] Saved training dynamics figure to: {fig_path}")


# -----------------------------
# main：多层数、多 seed，Baseline vs Mix
# -----------------------------
def main():
    ...
    parser.add_argument('--layers_list', type=str, default='3,5,8',
                        help='一次性跑多个层数，如 "3,5,8"；非空时忽略 --layers')
    ...
    parser.add_argument('--beta_list', type=str, default="0.95,0.9,0.85,0.8,0.75",
                        help='仅在 beta_mode=custom 时起作用；多个层数实验时会按当前 layers 检查长度')
    parser.add_argument('--C', type=float, default=1.05)

    parser.add_argument('--tau', type=float, default=0.95)
    parser.add_argument('--abs_target', type=float, default=0.78)
    ...
    edge_index, edge_weight = gcn_norm(
        ...
        num_nodes=data.num_nodes, add_self_loops=True
    )

    # 边对称性检查（可解释 Dirichlet）
    inspect_edge_symmetry(edge_index, num_nodes=data.num_nodes, verbose=True)

    # 3. 对每个 layers 单独做一轮完整实验，并各自输出 CSV
    for L in layers_to_run:
        print(f"\n========== Running experiments for layers = {L} ==========")

        # 为当前层数构造 beta_list（只用于 mix 模型）
        beta_list = None
        if args.beta_mode == 'custom' and args.beta_list is not None and args.beta_list.strip():
            tmp = [float(x) for x in args.beta_list.split(',')]
            if len(tmp) != L:
                raise ValueError(
                    f"--beta_list 长度({len(tmp)})必须等于当前 layers({L})，"
                    f"当前正在跑的层数组合是 {layers_to_run}。"
                )
            beta_list = tmp
        else:
        ...
        summary_csv = os.path.join(args.outdir, f"summary_{args.dataset}_relu_L{L}.csv")
        curves_csv = os.path.join(args.outdir, f"curves_{args.dataset}_relu_L{L}.csv")
        ...
        print(f"\n=== Aggregate (robust stats over seeds, layers={L}) ===")
        print(table)

        print(f"\nSaved summary to: {summary_csv}")
        print(f"Saved curves to:  {curves_csv}")

        # 4. 画 Baseline vs Mix 的训练动态曲线（经典浅层可直接用）
        plot_dynamics_for_layers(curves_df, args.dataset, L, args.outdir)
