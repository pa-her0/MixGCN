...
class SAGELayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5,
        ...
        # 默认 PReLU（每层共享一个参数）
        self.activation = activation if activation is not None else nn.PReLU(num_parameters=1, init=0.1)
...
# -----------------------------
# Baseline-SAGE（内部默认 PReLU，最后一层无激活）
# -----------------------------
class BaselineSAGE(nn.Module):
    def __init__(self, in_dim, hidden, out_dim,
        ...
        for l in range(layers):
            ...
            act = (activation if activation is not None else nn.PReLU(num_parameters=1, init=0.1)) if l < layers - 1 else None
