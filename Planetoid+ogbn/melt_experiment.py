...
# Baseline-GCN vs MixGCN and ablation variants on WikiCS / Coauthor-CS / Coauthor-Physics
# 变体：
# - Baseline: 标准 GCN
# - Linear-only: 纯线性传播 X_{l+1} = A X_l W_l
# - Nonlinear-only: 仅缩放后的非线性路径 X_{l+1} ≈ 0.5 * sigma(A X_l W_l)
# - Full Mix: 我们的混合更新 X_{l+1} = beta * Z + (C - beta) * sigma(Z)
...
class GCNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=F.elu, cached: bool = True):
        super().__init__()
...
class MixGCNLayer(nn.Module):
    ...
    通用混合层：
        Z = \tilde{A} X_l W_l
        X_{l+1} = beta * Z + (C - beta) * sigma(Z)

    - 若 beta = 1.0, C = 1.0 => X_{l+1} = Z            （Linear-only）
    - 若 beta = 0.0, C = 0.5 => X_{l+1} = 0.5 * sigma(Z)（Nonlinear-only，最后一层仍为线性缩放）
    - 若 0 < beta < C      => 线性 + 非线性混合       （Full Mix）
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.2, dropout=0.5, activation=F.elu, cached: bool = True):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=cached, normalize=True, bias=True)
...
class BaselineGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=F.elu, cached: bool = True):
        super().__init__()
        assert layers >= 2
        mods = []
...
class MixGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
        ...
        for l in range(layers):
            ...
            mods.append(MixGCNLayer(idim, odim, beta=beta_l, C=C, dropout=dropout, activation=act, cached=cached))
        self.layers = nn.ModuleList(mods)
...
def build_model(cfg: RunConfig, in_dim, out_dim, use_cached: bool):
    ...
    四种变体：
      - baseline: 标准 GCN
      - linear: Linear-only  (beta=1.0, C=1.0)
      - nonlinear: Nonlinear-only (beta=0.0, C=0.5)
      - mix: Full Mix (beta schedule)
    """
    if cfg.model_type == 'baseline':
        return BaselineGCN(in_dim, cfg.hidden, out_dim,
                           dropout=cfg.dropout,
                           layers=cfg.layers,
                           cached=use_cached)

    elif cfg.model_type == 'linear':
        # Linear-only：所有层 beta=1.0, C=1.0
        return MixGCN(in_dim, cfg.hidden, out_dim,
                      dropout=cfg.dropout,
                      layers=cfg.layers,
                      beta=1.0,
                      C=1.0,
    ...
    elif cfg.model_type == 'nonlinear':
        # Nonlinear-only：隐层 0.5 * sigma(Z)，最后一层线性缩放
        return MixGCN(in_dim, cfg.hidden, out_dim,
                      dropout=cfg.dropout,
                      layers=cfg.layers,
                      beta=0.0,
...
def main():
    ...
    parser.add_argument('--beta_list', type=str, default="0.97,0.9,0.88,0.85,0.80,0.78,0.74,0.70",
                        help='逗号分隔；留空则按 beta_mode 自动生成（仅 mix 使用）')
    parser.add_argument('--C', type=float, default=1.1)

    parser.add_argument('--tau', type=float, default=0.90)
    parser.add_argument('--abs_target', type=float, default=0.85,
                        help='目标验证精度 (0~1)')
    ...
    # 四个变体：baseline / linear / nonlinear / mix
    for model_type in ['baseline', 'linear', 'nonlinear', 'mix']:
        for seed in args.seeds:
            ...
            print(f"[{ds_name}][{model_type}][seed={seed}] best_val={summary['best_val']:.4f} "
                  f"test_at_best_val={summary['test_at_best_val']:.4f} "
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{ds_name}_elu.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{ds_name}_elu.csv")
    ...
    def agg(group):
        ...
        return pd.Series({
            ...
            'test_at_best_val_mean': group['test_at_best_val'].mean(),
            'test_at_best_val_std': group['test_at_best_val'].std(),
