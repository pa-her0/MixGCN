...
# -----------------------------
# SAGE 基础层（激活函数改为 GReLU）
# -----------------------------
class SAGELayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation: Optional[nn.Module] = None, aggr: str = 'mean'):
        super().__init__()
        self.conv = SAGEConv(in_dim, out_dim, aggr=aggr, bias=True)
        self.dropout = nn.Dropout(dropout)
        self.activation = activation if activation is not None else GReLU()
...
# -----------------------------
# Baseline-SAGE（内部默认 GReLU，最后一层无激活）
# -----------------------------
