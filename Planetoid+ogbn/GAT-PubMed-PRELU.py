...
# -----------------------------
# 模型定义：Baseline-GAT 与 MixGAT (使用 PReLU)
# -----------------------------
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=None,
        ...
        # 默认使用 PReLU 激活
        self.activation = activation if activation is not None else nn.PReLU()
...
class MixGATLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.0, dropout=0.5, activation=None,
                 heads: int = 1, concat: bool = True, attn_dropout: float = 0.0,
                 negative_slope: float = 0.2, add_self_loops: bool = True):
        super().__init__()
        self.conv = GATConv(
...
class BaselineGAT(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=None):
        super().__init__()
        assert layers >= 2
        mods = []
        ...
        for l in range(layers):
            ...
            act = (activation if activation is not None else nn.PReLU()) if l < layers - 1 else None
...
class MixGAT(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
                 beta=0.5, C=1.0, activation=None, beta_list: Optional[List[float]] = None):
        super().__init__()
        assert layers >= 2
        dims = [in_dim] + [hidden] * (layers - 1) + [out_dim]
...
def main():
    ...
    # 数据（GAT 不需要 gcn_norm 预归一化；NormalizeFeatures 仍然保留）
    ...
    edge_weight = None                                # ✅ GAT 不使用 edge_weight
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{args.dataset}_PRELU.csv")
    curves_csv = os.path.join(args.outdir, f"curves_{args.dataset}_PRELU.csv")
