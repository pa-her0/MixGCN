...
# 若想用多头注意力（例如 heads=8），你可以在 GATLayer / MixGATLayer 的 __init__ 中把
# heads=8, concat=True，同时把下一层的 in_dim 
# 改成 hidden * heads（或者把 concat=False 并把 out_dim 设为目标通道数/heads）。
# 当前版本为保持与你的原实验维度一一致，默认使用 heads=1。
...
from torch_geometric.nn import GATConv  # ✅ 改为 GAT
...
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=F.relu,
        ...
        super().__init__()
        # 说明：
        # - 为了保持与原 GCN 宽度一致，默认 heads=1, concat=True（这时 out_dim 不变）
        # - 需要多头时，可将 heads>1；若保持通道不变，可设 concat=False 并把 out_dim 调大到 desired/out_heads
        self.conv = GATConv(
            in_dim, out_dim,
...
class BaselineGAT(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=F.relu):
        ...
        for l in range(layers):
            ...
            mods.append(GATLayer(idim, odim, dropout=dropout, activation=act))
...
class MixGAT(nn.Module):
    ...
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2,
        ...
        for l in range(layers):
            ...
            mods.append(MixGATLayer(idim, odim,
                                    beta=beta_l,
                                    C=C,
                                    dropout=dropout,
                                    activation=act))
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        return BaselineGAT(in_dim, cfg.hidden, out_dim, dropout=cfg.dropout, layers=cfg.layers)
    elif cfg.model_type == 'mix':
        return MixGAT(in_dim, cfg.hidden, out_dim, dropout=cfg.dropout, layers=cfg.layers,
...
def main():
    ...
    parser.add_argument('--beta_list', type=str, default="0.9,0.87,0.85")
    parser.add_argument('--C', type=float, default=1)

    parser.add_argument('--tau', type=float, default=0.95)
    parser.add_argument('--abs_target', type=float, default=0.69)
    ...
    for L in layers_to_run:
        ...
        if args.beta_mode == 'custom' and args.beta_list is not None and args.beta_list.strip():
            ...
            if len(tmp) != L:
                raise ValueError(
                    ...
                    f"当前 layers_list = {layers_to_run}"
                )
            beta_list = tmp
        else:
        ...
        # 文件名带上 L
        summary_csv = os.path.join(args.outdir, f"summary_GAT_{args.dataset}_relu_L{L}.csv")
        curves_csv = os.path.join(args.outdir, f"curves_GAT_{args.dataset}_relu_L{L}.csv")
