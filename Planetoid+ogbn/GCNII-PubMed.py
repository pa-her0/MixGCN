# 注意:
# - 将 Backbone 改为 GCNII（PyG 的 GCN2Conv），其余训练/指标逻辑保持不变。
# - baseline: 标准 GCNII（in_lin → 多层 GCN2Conv → out_lin）。
# - mix: 在每层 GCN2Conv 的输出 z 上做 beta/C 混合；最后一层用 C*z；再经 out_lin 分类。
# - Dirichlet 能量仍对分类器前的表示（最后一层 GCN2Conv 后）计算。
# - 如需逐层不同 beta，请用 --beta_list，长度需等于 --layers。
...
def main():
    ...
    parser.add_argument('--layers', type=int, default=16, help='GCNII 的 GCN2Conv 层数（不含首尾线性层）')
    ...
    parser.add_argument('--beta_mode', type=str, default='linear', choices=['custom', 'linear', 'exp', 'const'],
    ...
    parser.add_argument('--abs_target', type=float, default=0.8)
    ...
    # 数据（GCNII 不需要 gcn_norm 预归一化；NormalizeFeatures 仍然保留）
    ...
    summary_csv = os.path.join(args.outdir, f"summary_{args.dataset}.csv")
