...
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, activation=None,
        ...
        self.activation = activation if activation is not None else GRELU()  # 改成 GRELU
...
class BaselineGAT(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, activation=None):
        ...
        for l in range(layers):
            ...
            act = (activation if activation is not None else GRELU()) if l < layers - 1 else None
...
def main():
    ...
    parser.add_argument('--beta_list', type=str, default="0.95,0.87,0.85")
