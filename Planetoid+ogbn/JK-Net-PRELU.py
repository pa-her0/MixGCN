...
# -----------------------------
# Baseline JKNet (PReLU 版本)
# -----------------------------
class BaselineJKNet(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=3, mode='cat'):
        super().__init__()
        assert layers >= 2
        self.dropout = nn.Dropout(dropout)
        self.activation = nn.PReLU()   # 使用 PReLU
        self.mode = mode
...
class MixJKLayer(nn.Module):
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.0, dropout=0.5):
        super().__init__()
...
class MixJKNet(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, layers=3,
                 ...
                 mode='cat', beta_list: Optional[List[float]] = None):
        super().__init__()
        assert layers >= 2
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        ...
        return MixJKNet(
            ...
            dropout=cfg.dropout, mode=cfg.jk_mode,beta_list=betas
        )
    else:
