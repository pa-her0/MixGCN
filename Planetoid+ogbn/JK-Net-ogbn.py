...
def inspect_edge_symmetry(edge_index: torch.Tensor, num_nodes: int, verbose: bool = True) -> float:
    ...
    if verbose:
        print(f"[Info] Edge symmetry ratio (approx, ignore self-loops): {ratio:.3f} "
              f"(1.0≈双向成对；<1.0≈可能单向存储). Dirichlet energy is for relative comparison.")
    return ratio
...
def load_dataset(name: str, root: str = 'runs/data'):
    """
    返回 (data, raw_edge_index)
      - data: 包含 x, y, train/val/test_mask（均为 1D bool）
      - raw_edge_index: 原始边（LongTensor[2, E]）；后续在 main 里统一用 gcn_norm 处理
    """
    nlow = name.lower()
    ...
    if nlow in ['ogbn-arxiv', 'ogbnarxiv', 'arxiv']:
        ...
        data.edge_index = ToUndirected()(data).edge_index  # 转无向

        return data, data.edge_index

    if nlow in ['chameleon', 'cham', 'camel']:
...
class BaselineJKNet(nn.Module):
    ...
    """
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=3, mode='cat', activation=F.leaky_relu):
        super().__init__()
        assert layers >= 2
...
class MixJKLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.0, dropout=0.5, activation=F.leaky_relu):
        super().__init__()
...
class MixJKNet(nn.Module):
    ...
    def __init__(self, in_dim, hidden, out_dim, layers=3,
                 ...
                 mode='cat', activation=F.leaky_relu, beta_list: Optional[List[float]] = None):
        super().__init__()
        assert layers >= 2
...
def build_model(cfg: RunConfig, in_dim, out_dim):
    ...
    if cfg.model_type == 'baseline':
        return BaselineJKNet(
            ...
            mode=cfg.jk_mode, activation=F.leaky_relu
        )
        ...
        return MixJKNet(
            ...
            activation=F.leaky_relu, beta_list=betas
        )
    else:
...
def main():
    ...
    # 加载数据 + gcn_norm 预归一化（与 GCNConv(normalize=False) 配套）
    data, raw_edge_index = load_dataset(args.dataset, root=os.path.join(args.outdir, 'data'))
    edge_index, edge_weight = gcn_norm(
        raw_edge_index, None, num_nodes=data.num_nodes, add_self_loops=True
    )
    ...
    summary_csv = os.path.join(args.outdir, f"JK-Net_summary_{args.dataset}.csv")
