...
# -----------------------------
# 模型定义：Baseline 与 MixGCN (PReLU 激活版)
# -----------------------------
class GCNLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.5, use_prelu=True):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=False, normalize=False, bias=True)
        self.dropout = nn.Dropout(dropout)
        # 如果需要 PReLU：每个输出维度一个参数
        self.activation = nn.PReLU(num_parameters=out_dim) if use_prelu else None
...
class MixGCNLayer(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, beta=0.5, C=1.0, dropout=0.5, use_prelu=False):
        super().__init__()
        self.conv = GCNConv(in_dim, out_dim, cached=False, normalize=False, bias=True)
        self.beta = float(beta)
        self.C = float(C)
        self.dropout = nn.Dropout(dropout)
        self.activation = nn.PReLU(num_parameters=out_dim) if use_prelu else F.relu
...
class BaselineGCN(nn.Module):
    def __init__(self, in_dim, hidden, out_dim, dropout=0.5, layers=2, use_prelu=True):
        super().__init__()
        assert layers >= 2
        mods = []
        ...
        for l in range(layers):
            ...
            # 最后一层不用激活
            act = use_prelu if l < layers - 1 else False
            mods.append(GCNLayer(idim, odim, dropout=dropout, use_prelu=act))
...
def main():
    ...
    parser.add_argument('--beta_list', type=str, default="0.9,0.85,0.8,0.75,0.7")
    parser.add_argument('--C', type=float, default=1.2)

    parser.add_argument('--tau', type=float, default=0.95)
    parser.add_argument('--abs_target', type=float, default=0.67)
