...
# -----------------------------
# 模型定义：Baseline 与 MixGCN（Swish 版）
# -----------------------------
class GCNLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int,
        ...
        self.activation = nn.SiLU() if use_swish else None  # Swish 激活
...
class BaselineGCN(nn.Module):
    def __init__(self, in_dim: int, hidden: int, out_dim: int,
        ...
        for l in range(layers):
            ...
            # 最后一层不用激活
            use_act = (l < layers - 1)
            mods.append(GCNLayer(idim, odim, dropout=dropout, use_swish=use_act))
...
def main():
    ...
    parser.add_argument('--abs_target', type=float, default=0.58)
